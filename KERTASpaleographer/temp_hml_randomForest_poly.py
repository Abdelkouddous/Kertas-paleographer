...
predictions_RF_poly=rf.predict(Xt)
#post traitement
accuracy_randomForest = accuracy_score(Yt, predictions_RF_poly)*100#Varience 
print('best score is : ' , accuracy_randomForest, '\n')
print(classification_report(Yt, predictions_RF_poly))
print(confusion_matrix(Yt,predictions_RF_poly))
#print(f"Best random forest classifier score: {score_randomForest}" )
'''random state est plus lourd quand on l'augemente mais le taux d'err diminue
c'est pas le cas pour l'estmator '''
'''PLOTTING
'''
matrix=confusion_matrix(Yt,predictions_RF_poly)
...
plt.pyplot.title('Confusion Matrix for RandomForest Model using Polygon FE')
plt.pyplot.show()
print('========= End  =========')
# print classification report
classif_report=classification_report(Yt, predictions_RF_poly)
#classif_report=matrix.astype('float')
def plot_classification_report(cr, title='Classification report for RandomForest model using Polygon FE ', with_avg_total=False, cmap=plt.cm.Blues):
    lines = cr.split('\n')
    classes = []
    plotMat = []
