...
features_testing = zeros(605);
    ...
    for i=1:nfile
      ...
      %}
      features_testing (i,:) = ChainCode_GlobalFeatures_Extraction_PRL(inf);
      if isnan(features_testing)
         features_testing(i,:)=abs(rand(0.05,0.7)*cos(i));
      end
      csvwrite('features_testing_chainCodeGlobalFE.csv', features_testing);      
     end    
toc;